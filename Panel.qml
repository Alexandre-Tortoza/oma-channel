...
import "Model.js" as Model
 ...
 Panel {
  ...
  ipcTarget: "io.github.alexmrtr.oma-channel"
  manageIpc: false
  ...
  // Opens straight into the reader with the "Saved" filter applied. Called by
  // BarWidget.qml either from a middle-click affordance or the IPC
  // "bookmarks" command relayed through Service.qml.
  function openBookmarks() {
    if (!root.opened) root.open()
    root.currentView = "reader"
    readerView.bookmarkedOnly = true
  }
      ...
      Keys.onPressed: function(event) {
        ...
        if (root.currentView === "reader") {
          if (event.key === Qt.Key_J || event.key === Qt.Key_Down) {
            ...
            readerView.toggleBookmarkCurrentArticle()
            event.accepted = true
