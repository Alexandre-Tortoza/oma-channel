...
2. **QML frontend** — presentation only, never parses XML:
   - `Service.qml` (manifest kind `"service"`, singleton): owns every `Process` spawn (fetch/list/mark/bookmark/enrich-artwork/tray/OPML), the in-memory article cache, settings, and the `IpcHandler` (`target: "io.github.alexmrtr.oma-channel"`, commands `refresh`/`markAllRead`/`cleanup`/`bookmarks`/`status`). One instance regardless of monitor count — a bar surface is built per monitor, so anything that fetches needs to live here, not in `BarWidget.qml`, or it fetches once per screen. Also owns the `trayIconEnabled` setting and the long-running `oma-channel tray` process (bounded restart-with-backoff on unexpected exit, e.g. no D-Bus tray host yet).
   - When `trayIconEnabled` is on, `BarWidget.qml` swaps its `BarIconButton` for a near-zero (`implicitWidth/Height: 2`) anchor `Item` instead of `visible: false` — a fully hidden widget collapses the bar's `ModuleSlot` to zero width, which degrades `Panel.qml`'s popup anchoring to an arbitrary point between neighboring widgets (confirmed by reading `Bar.qml`'s `ModuleSlot` and `PopupCard.qml`'s anchor math). The tiny anchor keeps a stable position while still freeing almost all the bar space.
...
- Settings keys: `subscriptions`, `pollIntervalMinutes`, `maxItemsPerFeed`, `itemsPerPage`, `retentionDays`, `unreadOnlyDefault`, `barSection`, `artworkEnabled`, `artworkAllowPageFetch`, `trayIconEnabled`.
- The tray icon (`src/tray.rs`) publishes a StatusNotifierItem via the `ksni` crate's blocking API — no async runtime management needed in our own code, `TrayMethods::spawn()`/`Handle::update()` are plain sync calls. Icon pixels are generated at runtime (a solid circle, ARGB32/network-byte-order per `ksni::Icon`'s docs) rather than shipped as an asset. Clicking the icon or its menu items shells out to `omarchy-shell` with the same IPC surface documented below — no new IPC path between the daemon and `Service.qml`.
